...
Updater signing keys are loaded automatically from `.env.release.local` when
present. This file is ignored by Git because `.gitignore` excludes `*.local`.
Create it once on your release machine:

```bash
TAURI_SIGNING_PRIVATE_KEY=/Users/you/.tauri/nikon-connector-updater.key
TAURI_SIGNING_PRIVATE_KEY_PASSWORD=optional-password
```

`TAURI_SIGNING_PRIVATE_KEY` may point to the private key file generated by
`bun tauri signer generate`. Keep the matching public key in
`src-tauri/tauri.conf.json`.
